			...
			if (null != bookInfo) {
				...
				if (null != fileInfo) {
					...
					if (null != data) {
						...
						String fileName = fileInfo.getFilename() + "_" + fileInfo.crc32 + ".bmk.xml.gz";
			...
			if (null != bookInfo) {
				...
				if (null != fileInfo) {
					try {
						...
						props.setProperty("filename", fileInfo.getFilename());
					...
					if (null != inputStream) {
						try {
							...
							if (CURRENTBOOKINFO_BUNDLE_VERSION == version) {
								...
								fileInfo.setFilename(props.getProperty("filename"));
	...
	private byte[] getCurrentBookBookmarksData(BookInfo bookInfo) {
		...
		if (null != fileInfo) {
			try {
				...
				serializer.text(fileInfo.getFilename());
	...
	private void syncBookmarks(InputStream inputStream) {
		...
		if (null != fileInfo && null != bookmarks) {
			...
			BackgroundThread.instance().executeGUI(() -> m_coolReader.waitForCRDBService(() -> {
				//m_coolReader.getDB().findByFingerprint(2, finalfileInfo.getFilename(), finalFileInfo.crc32,
