...
import org.coolreader.crengine.StatusBar;
...
public class ReaderViewLayout extends ViewGroup implements Settings {
		...
		public View getSepBottom() {
			return sepBottom;
		}

		public void clearSepBottom() {
			if (sepBottom != null) {
				sepBottom.setVisibility(View.INVISIBLE);
			}
		}
...
//				});
		...
		public ReaderViewLayout(CoolReader context, ReaderView contentView) {
			...
			if (DeviceInfo.EINK_SCREEN) {
				sepBottom = new View(activity);
				llBottom.addView(sepBottom);
			}
			this.addView(llLeft);
...
//			}
		...
		//	};
			...
			int margb = marg == 0? (DeviceInfo.EINK_SCREEN? 1: 0): marg; // for eink always put indicator to bottom - for autoscroll
			//margb = 0;
			...
			if ((marg > 0) || (margb > 0)) {
				clearSepBottom();
				llBottom.layout(full_l, full_b - margb, full_r, full_b);
				if (sepBottom != null) sepBottom.layout(0, margb-1, 1, margb);
			}
