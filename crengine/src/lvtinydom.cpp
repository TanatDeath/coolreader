...
#define CACHE_FILE_FORMAT_VERSION "3.12.85"
...
//        if ( _currNode ) {
//        lString32 sname = _currNode->getElement()->getAttributeValue( "name" );
//            if ( !sname.empty() ) {
...
lString32 extractDocAuthorsExt( ldomDocument * doc, lString32 delimiter1, lString32 delimiter)
{
    ...
    for ( int i=0; i<16; i++) {
        ...
        lString32 author = firstName;
        lString32 author_full = firstName;
        author += delimiter1;
...
lString32 extractDocAuthorsAddInfo( ldomDocument * doc, lString32 delimiter, bool shortMiddleName )
{
    if ( delimiter.empty() )
        delimiter = ", ";
    lString32 authors_add_info;
    for ( int i=0; i<16; i++) {
        lString32 path = cs32("/FictionBook/description/title-info/author[") + fmt::decimal(i+1) + "]";
        ldomXPointer pauthor = doc->createXPointer(path);
        if ( !pauthor ) {
        ...
        lString32 homePage = pauthor.relative( U"/home-page" ).getText().trim();
        lString32 email = pauthor.relative( U"/email" ).getText().trim();
        lString32 addInfo = lString32::empty_str;
...
lString32 extractDocGenre( ldomDocument * doc, lString32 delimiter)
{
    if ( delimiter.empty() )
        delimiter = ", ";
    lString32 genres;
    for ( int i=0; i<16; i++) {
        ...
        lString32 genre = pgenre.getText().trim();
        if ( !genres.empty() )
            genres += delimiter;
        genres += genre;
    }
...
lString32 extractDocAnnotation( ldomDocument * doc )
{
    ldomNode * body = doc->nodeFromXPath(cs32("/FictionBook/description/title-info/annotation[1]"));
    lString32 res;
    res = "";
    if (body != NULL) {
        ...
        for (int i = 0; i < cnt; i++) {
            ...
            if (!l1section)
                continue;
            lString32 res2 = l1section->getText();
            if (res.length()==0) res = res2; else
                res = res + "\n" + res2;
        }
        if (cnt==0) return doc->createXPointer(U"/FictionBook/description/title-info/annotation").getText();
    }
    return res;
}

lString32 extractDocSrcLang( ldomDocument * doc )
{
    return doc->createXPointer(U"/FictionBook/description/title-info/src-lang").getText();
}

lString32 extractDocBookDate( ldomDocument * doc )
{
    return doc->createXPointer(U"/FictionBook/description/title-info/date").getText();
}


lString32 extractDocTranslator( ldomDocument * doc, bool shortMiddleName  )
{
    lString32 firstName = doc->createXPointer(U"/FictionBook/description/title-info/translator/first-name").getText().trim();
    lString32 lastName = doc->createXPointer(U"/FictionBook/description/title-info/translator/last-name").getText().trim();
    lString32 middleName = doc->createXPointer(U"/FictionBook/description/title-info/translator/middle-name").getText().trim();
    lString32 translator = firstName;
    if ( !translator.empty() )
      translator += " ";
    if ( !middleName.empty() )
      translator += shortMiddleName ? lString32(middleName, 0, 1) + "." : middleName;
...
lString32 extractDocAuthor( ldomDocument * doc )
{
    lString32 nickName = doc->createXPointer(U"/FictionBook/description/document-info/author/nickname").getText();
    lString32 email = doc->createXPointer(U"/FictionBook/description/document-info/author/email").getText();
...
lString32 extractDocProgram( ldomDocument * doc )
{
    return doc->createXPointer(U"/FictionBook/description/document-info/program-used").getText();
}

lString32 extractDocDate( ldomDocument * doc )
{
    return doc->createXPointer(U"/FictionBook/description/document-info/date").getText();
}

lString32 extractDocSrcUrl( ldomDocument * doc )
{
    return doc->createXPointer(U"/FictionBook/description/document-info/src-url").getText();
}

lString32 extractDocSrcOcr( ldomDocument * doc )
{
    return doc->createXPointer(U"/FictionBook/description/document-info/src-ocr").getText();
}

lString32 extractDocVersion( ldomDocument * doc )
{
    return doc->createXPointer(U"/FictionBook/description/document-info/version").getText();
}

lString32 extractPublName( ldomDocument * doc )
{
    return doc->createXPointer(U"/FictionBook/description/publish-info/book-name").getText();
}

lString32 extractPublisher( ldomDocument * doc )
{
    return doc->createXPointer(U"/FictionBook/description/publish-info/publisher").getText();
}

lString32 extractPublCity( ldomDocument * doc )
{
    return doc->createXPointer(U"/FictionBook/description/publish-info/city").getText();
}

lString32 extractPublYear( ldomDocument * doc )
{
    return doc->createXPointer(U"/FictionBook/description/publish-info/year").getText();
}

lString32 extractPublIsbn( ldomDocument * doc )
{
    return doc->createXPointer(U"/FictionBook/description/publish-info/isbn").getText();
}
...
lString32 extractDocPublishSeries( ldomDocument * doc, int * pSeriesNumber ) {
    lString32 res;
...
//        lChar32 nextCh = (i+1)<textLen ? text[i+1] : 0;
//        lChar32 nextCh2 = (i+2)<textLen ? text[i+2] : 0;
//        lChar32 nextCh3 = (i+3)<textLen ? text[i+3] : 0;
...
bool ldomXPointerEx::isSentenceEnd()
{
    ...
    lChar32 currCh = i<textLen ? text[i] : 0;
    lChar32 prevCh = i>0 ? text[i-1] : 0;
    lChar32 prevCh2 = i>1 ? text[i-2] : 0;
    lChar32 prevCh3 = i>2 ? text[i-3] : 0;
    lChar32 prevCh4 = i>3 ? text[i-4] : 0;
    //lChar32 nextCh = (i+1)<textLen ? text[i+1] : 0;
