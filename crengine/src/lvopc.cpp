        ...
        if( doc ) {
            ...
            if(root) {
                for(int i = 0; i < root->getChildCount(); i++) {
                    ...
                    if(typeNode->getNodeName() == cs32("Override")) { //Don't care about Extensions
                        lString32 contentType = typeNode->getAttributeValue(U"ContentType");
                        lString32 partName = typeNode->getAttributeValue(U"PartName");
                        if (
                                (partName == U"/word/document.xml")&&
                                (contentType == U"application/textmaker.wordprocessingml.document.main+xml")
                           ) {
                            m_contentTypes.set(U"application/vnd.openxmlformats-officedocument.wordprocessingml.document.main+xml",
